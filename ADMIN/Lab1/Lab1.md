...
5. Включите пользователя *vinnie* в группу *vgroup*.
    ...
    СДЕЛАТЬ КОМАНДОЙ ОТ ИМЕНИ ПОЛЬЗОВАТЕЛЯ
...
3. Измените начальный интерпретатор пользователя на **csh**.
    ...
    CSH сменить из под пользователя так же
...
1. Задайте следующую политику учетной записи *vinnie*:
    ...
    Нужную политику можно установить с помощью параметров команды **chage**, а также изменив файл **/etc/shadow**. Здесь рассмотрим как это производится изменением файла (о том как изменить политику используя **chage** можно почитать в мануале команды). Изменим строку в **/etc/shadow**:
    ```console
